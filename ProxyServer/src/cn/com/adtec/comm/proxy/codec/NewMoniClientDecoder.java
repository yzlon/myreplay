...
public class NewMoniClientDecoder extends CumulativeProtocolDecoder {
	private final Charset charset;
	private final AttributeKey CONTEXT = new AttributeKey(getClass(), "context");
	private final static Logger log = LoggerFactory	.getLogger(NewMoniClientDecoder.class);
	
	

	public NewMoniClientDecoder(Charset charset) {
	...
	protected boolean doDecode(IoSession session, IoBuffer in,
		...
		try{
			while (in.hasRemaining()) {
				...
				currLen+=thisLimit;
				buffer.put(b);
				//System.out.print("totleLen["+totleLen+"]_____两者之和["+(NewMoniObject.COMMLEN_LENGTH+co.getBodyLen()-1)+"]");
			...
			if (co.isPubHeadOkFlag() )
			{
				if (co.getTyp().equals("1"))
				{
					...
					if (totleLen==NewMoniObject.COMMLEN_LENGTH+co.getHeadLen())
					{
						...
						//System.out.println("在解码新监控返回报文时获取的整个报文：【"+co.getInnerBuffer().toString()+"】");
						out.write(newObj);
						newObj=null;
						//out.write(co);
						co.reset();
				...
				else if (co.getTyp().equals("0"))
				{
					...
					if(!co.isHeadOkFlag() && totleLen>=NewMoniObject.COMMLEN_LENGTH + co.getHeadLen())
					{
						...
						//System.out.println("totleLen["+totleLen+"]currLen["+currLen+"]MD5：["+co.getHead().getMD5()+"]");
						
						//buffer.position(totleLen-currLen+NewMoniObject.DATA_HEAD_PACK_FLG_LEN+NewMoniObject.DATA_HEAD_LEFT_TIME_OUT_LEN+NewMoniObject.DATA_HEAD_MD5_LEN);
								...
								+NewMoniObject.DATA_TXCD_LEN+co.getBodyLen() && co.getBodyLen()!=0 )
					{
						...
						newObj.setTyp(co.getTyp());
						newObj.setTotalLen(co.getTotalLen());
						//System.out.println("在解码时获取的整个报文：【"+co.getInnerBuffer().toString()+"】");
						out.write(newObj);
						newObj=null;
						//out.write(co);
