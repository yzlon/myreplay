...
import cn.com.adtec.comm.proxy.handler.SubXMLServerBusinessHandler;
 ...
 *
 */
public class SubXMLServer extends AbstractSubServer {
	
	private static final Logger logger = LoggerFactory.getLogger(SubXMLServer.class);
	/**
	 * 刷新服务器属性相关配置
	...
	public boolean startSubServer() {
		if (this.getAcceptor()==null)
		{
			...
			try {
				...
				this.getAcceptor().setHandler(new SubXMLServerBusinessHandler(this));
				...
				logger.error("Start SubXMLServer fail,the reason is....\n"+e.toString());
				e.printStackTrace();
				return false;
			}
		}//if (acceptor==null)
